...
from dino_train_processes import *
from tools import *
...
def prompt_generation(model, val_loader, i, pretrain_name):
    pretrain_path = r'/mnt/e/Mr.Wu/codes/Semi-Supervised-Camouflaged-Object-Detection-with-Scribble-Annotations-main/out/trained/'
    pretrain = os.path.join(pretrain_path, pretrain_name)
...
MAX_LR = 5e-3
MAX_LR_SAM = 5e-4
EXP_NAME = '/mnt/e/Mr.Wu/codes/Weakly-Supervised-Camouflaged-Object-Detection-with-Scribble-Annotations-main/out\\new' # change it in main
root = '/mnt/e/Mr.Wu/dataset/Semi-CodDataset'
pretrain_name = 'dino_resmamba_41.pth'
...
def train(Dataset, net, cfg1, cfg2, cfg3, prompt, train_loss, start_from = 0):
    ...
    for epoch in range(start_from, total_epoch):
        ...
        if epoch >= (cfg1.epoch + cfg2.epoch) and epoch % 100 == 0:
            ...
            i = prompt_generation(net, prompt_loaders, i, pretrain_name)
            torch.cuda.empty_cache()
            ...
            optimizer3.load_state_dict(torch.load(cfg1.savepath + '/optimizer_' + pretrain_name))
        # stage 1
        if epoch < cfg1.epoch:
            prefetcher = DataPrefetcher(stage1_loader, cfg1.mode)
            batch_idx1 = -1
            image, mask = prefetcher.next(cfg1.mode)
            while image is not None:
                ...
                loss = train_loss([image, mask], cfg1.mode, net, dict(epoch=epoch+1, global_step=global_step1, sw=sw1, t_epo=cfg1.epoch))
                ...
                if batch_idx1 % 1000 == 0:
                    ...
                    print(msg)
                    logger.info(msg)
            if (epoch + 1) % cfg1.epoch == 0:
                # torch.save(net.state_dict(), cfg1.savepath + '/cnn_nosam.pth')
                # torch.save(optimizer1.state_dict(), cfg1.savepath + '/optimizer_cnn_nosam.pth')
                ...
                if mae < min_mae:
                    min_mae = mae
                    best_epoch = epoch + 1
                    torch.save(net.state_dict(), cfg1.savepath + '/' + pretrain_name)
                    torch.save(optimizer1.state_dict(), cfg1.savepath + '/optimizer_' + pretrain_name)
        ...
        if epoch >= (cfg1.epoch + cfg2.epoch):
            ...
            while image is not None:
                ...
                if batch_idx3 % 100 == 0:
                    ...
                    print(msg)
                    logger.info(msg)
            if (epoch + 1) % (cfg1.epoch + cfg2.epoch + cfg3.epoch) == 0:
                mae = validate_multiloader(net, val_loaders1)
                print('VAL MAE:%s' % (mae))
                ...
                if mae < min_mae:
                    min_mae = mae
                    best_epoch = epoch + 1
                    torch.save(net.state_dict(), cfg3.savepath + '/' + pretrain_name)
                    torch.save(optimizer3.state_dict(), cfg1.savepath + '/optimizer_' + pretrain_name)
                    print('best epoch is:%d, MAE:%s' % (best_epoch, min_mae))
...
if __name__=='__main__':
    def parse_args():
        ...
        parser.add_argument('--pretrain', type=str,
                            ...
                            # default=r'/mnt/e/Mr.Wu/codes/Semi-Supervised-Camouflaged-Object-Detection-with-Scribble-Annotations-main/out/trained/cnn_41.pth',
    ...
    from model.DinoCamoFormer import CamoFormer
