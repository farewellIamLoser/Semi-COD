...
from train_processes_FSP import *
from tools import *
...
def prompt_generation(model, val_loader, i):
    pretrain = r'/mnt/e/Mr.Wu/codes/Semi-Supervised-Camouflaged-Object-Detection-with-Scribble-Annotations-main/out/trained/SINet.pth'
...
def train(Dataset, net, cfg1, cfg2, cfg3, prompt, train_loss, start_from = 0):
    ...
    val_cfg1 = [Dataset.Config(datapath=f'{root}/TestDataset/{i}', mode='test') for i in ['COD10K', 'N4CK', 'CAMO', 'CHAMELEON']]
    ...
    val_cfg2 = [Dataset.Config(datapath=f'{root}/TestDataset/{i}', mode='test') for i in ['COD10K', 'N4CK', 'CAMO', 'CHAMELEON']]
    ...
    for epoch in range(start_from, total_epoch):
        ...
        #     prompt_generation(net, prompt_loaders)
        ...
        elif epoch == cfg1.epoch and cfg2.epoch > 0:
            base, head = [], []
            for name, param in net.named_parameters():
                if 'encoder' in name:
                    base.append(param)
                    ...
                    head.append(param)
            optimizer2 = torch.optim.SGD([{'params': base}, {'params': head}], lr=cfg2.lr, momentum=cfg2.momen,
                                         weight_decay=cfg2.decay, nesterov=True)
        elif epoch >= (cfg1.epoch + cfg2.epoch) and epoch % 100 == 0:
        ...
        if epoch < cfg1.epoch:
            ...
            if (epoch + 1) % 600 == 0:
                ...
                if mae < min_mae:
                    min_mae = mae
                    best_epoch = epoch + 1
                    torch.save(net.state_dict(), cfg1.savepath + '/SINet.pth')
                    # torch.save(optimizer1.state_dict(), cfg1.savepath + '/cnn_baseline_optimizer.pth')
                    print('best epoch is:%d, MAE:%s' % (best_epoch, min_mae))

        # stage 2
        elif cfg1.epoch <= epoch < (cfg1.epoch + cfg2.epoch) and cfg2.epoch > 0:
            prefetcher = DataPrefetcher(stage2_loader, cfg2.mode)
            batch_idx2 = -1
            image, mask, Simage, Smask, label = prefetcher.next(cfg2.mode)
            while image is not None:
                st2 = time.time()
                niter2 = (epoch - cfg1.epoch) * db_size2 + batch_idx2
                lr2, momentum2 = get_triangle_lr(BASE_LR, MAX_LR, cfg2.epoch * db_size2, niter2, ratio=1.)
                optimizer2.param_groups[0]['lr'] = 0.1 * lr2  # for backbone
                optimizer2.param_groups[1]['lr'] = lr2
                optimizer2.momentum = momentum2
                batch_idx2 += 1
                global_step2 += 1

                loss = train_loss([image, mask, Simage, Smask, label], cfg2.mode, net,
                                                               dict(epoch=epoch + 1, global_step=global_step2, sw=sw2,
                                                                    t_epo=cfg2.epoch))
                ...
                loss.backward()
                optimizer2.step()
                sw2.add_scalar('lr', optimizer2.param_groups[0]['lr'], global_step=global_step2)
                sw2.add_scalar('loss', loss.item(), global_step=global_step2)

                image, mask, Simage, Smask, label = prefetcher.next(cfg2.mode)
                ta2 = time.time() - st2
                et2 = 0.9 * et2 + 0.1 * ta2 if et2 > 0 else ta2
                if batch_idx2 % 10 == 0:
                    msg = '%s| %s | eta:%s | step:%d/%d/%d | lr=%.6f | loss=%.6f | bestepoch=%d' % (
                    TAG, datetime.datetime.now(), datetime.timedelta(seconds=int((cfg2.epoch * db_size2 - niter2) * et2)),
                    global_step2, epoch + 1, cfg2.epoch, optimizer2.param_groups[0]['lr'], loss.item(), best_epoch)
                    print(msg)
                    logger.info(msg)
            if (epoch+1)%50==0:
                mae = validate_multiloader(net, val_loaders2)
                print('VAL MAE:%s' % (mae))
                sw2.add_scalar('val', mae, global_step=global_step2)
                if mae < min_mae :
                    min_mae = mae
                    best_epoch = epoch + 1
                    torch.save(net.state_dict(), cfg2.savepath + '/SINet.pth')
                    # torch.save(optimizer2.state_dict(), cfg2.savepath + '/cnn_baseline_optimizer.pth')
                    print('best epoch is:%d, MAE:%s' % (best_epoch, min_mae))

        # stage 3 SAM pesudo training
        elif epoch >= (cfg1.epoch + cfg2.epoch):
            ...
            if (epoch + 1) % 100 == 0:
                ...
                if mae < min_mae:
                    min_mae = mae
                    best_epoch = epoch + 1
                    torch.save(net.state_dict(), cfg3.savepath + '/SINet.pth')
                    # torch.save(optimizer3.state_dict(), cfg3.savepath + '/cnn_baseline_optimizer.pth')
                    print('best epoch is:%d, MAE:%s' % (best_epoch, min_mae))

        if epoch == cfg3.epoch-2 or epoch == cfg3.epoch-1 or (epoch+1) % 30 == 0:
            torch.save(net.state_dict(), cfg3.savepath + '/model-' + str(epoch + 1))
...
if __name__=='__main__':
    def parse_args():
        ...
        parser.add_argument('--pretrain', type=str,
                            default=r'/mnt/e/Mr.Wu/codes/FSPNet_Weak/checkpoint/Backbone/base_patch16_384.pth',
    ...
    from model.SINet import SINet_ResNet50
    net = SINet_ResNet50(channel=32).cuda()

    # if args.pretrain:
    #     encoder = torch.load(args.pretrain)
    #     net.load_state_dict(encoder['model'], strict=False)
