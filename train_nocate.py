...
def prompt_generation(model, val_loader, i):
    pretrain = r'/mnt/e/Mr.Wu/codes/Semi-Supervised-Camouflaged-Object-Detection-with-Scribble-Annotations-main/out/trained/CamoMamba_808.pth'
        ...
        if epoch < cfg1.epoch:
            ...
            while image is not None:
                ...
                if batch_idx1 % 10 == 0:
                    ...
                    logger.info(msg)
            if (epoch + 1) % 30 == 0:
                torch.save(net.state_dict(), cfg3.savepath + '/model-4041cate' + str(epoch + 1))
                ...
                if mae < min_mae:
                    min_mae = mae
                    best_epoch = epoch + 1
                    torch.save(net.state_dict(), cfg1.savepath + '/CamoMamba_4041.pth')
            ...
            if (epoch + 1) % 100 == 0:
                ...
                if mae < min_mae:
                    min_mae = mae
                    best_epoch = epoch + 1
                    torch.save(net.state_dict(), cfg3.savepath + '/CamoMamba_4041.pth')
                    print('best epoch is:%d, MAE:%s' % (best_epoch, min_mae))
...
if __name__=='__main__':
    ...
    cfg1 = dataset_no.Config(datapath=f'{root}', savepath=f'./out/{EXP_NAME}/', mode='stage1', batch=args.batch_size, lr=1e-3, momen=0.9, decay=5e-4, epoch=stage1_epoch, label_dir = 'Scribble')
    cfg2 = dataset_no.Config(datapath=f'{root}', savepath=f'./out/{EXP_NAME}/', mode='stage2', batch=args.batch_size, lr=1e-3, momen=0.9, decay=5e-4, epoch=stage2_epoch, label_dir = 'Scribble')
    cfg3 = dataset_no.Config(datapath=f'{root}', savepath=f'./out/{EXP_NAME}/', mode='stage3', batch=args.batch_size, lr=1e-3, momen=0.9, decay=5e-4, epoch=stage3_epoch, label_dir = 'Scribble')
    prompt = dataset_no.Config(datapath=f'{root}', mode='prompt')
    seed_everything(42)
    ...
    train(dataset_no, net, cfg1, cfg2, cfg3, prompt, tm, start_from=0)