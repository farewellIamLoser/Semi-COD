...
class SFTA(nn.Module):
    def __init__(self, in_channel, out_channel, img_size, stride, type):
    ...
    def forward(self, x, y):
        ...
        states = torch.sigmoid(hidden_states)
        hidden_states_bg = hidden_states * (1 - states)
        hidden_states_fg = hidden_states * states
        hidden_states_n = hidden_states
        hidden_states = torch.cat((hidden_states_fg, hidden_states_bg, hidden_states_n), dim=2)
        init_states = self.activation(self.pos_linear(hidden_states))
        residual = None
        i = 0
        for layer in self.layers:
            if i == 0:
                hidden_states, residual = layer(init_states, residual)
            ...
            i += 1
            hidden_states = self.layer_norm(hidden_states) + self.layer_norm(init_states)
        out = layer_norm_fn(
            self.drop_path(hidden_states),
            self.norm_f.weight,
            self.norm_f.bias,
...
class Decoder(nn.Module):
    def __init__(self, channels, type='1'):
        super(Decoder, self).__init__()
        img_sizes = [10, 20, 40, 80, 320]

        self.pyramid_pooling = PyramidPooling(512, channels)
        self.sfa1 = SFTA(512, channels, img_size=img_sizes[0], stride=2, type=type)
        self.sfa2 = SFTA(320, channels, img_size=img_sizes[1], stride=4, type=type)
        self.sfa3 = SFTA(128, channels, img_size=img_sizes[2], stride=4, type=type)
        self.sfa4 = SFTA(64, channels, img_size=img_sizes[3], stride=4, type=type)
        ...
        # self.final_out = nn.Sequential(nn.Conv2d(channels * 4, channels, 1, bias=False),
