...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.sfa1 = SFTA(512, channels, img_size=img_sizes[0], stride=2)
        self.sfa2 = SFTA(320, channels, img_size=img_sizes[1], stride=4)
        self.sfa3 = SFTA(128, channels, img_size=img_sizes[2], stride=4)
        self.sfa4 = SFTA(64, channels, img_size=img_sizes[3], stride=4)
    ...
    def forward(self, E1, E2, E3, E4, img):
        S5 = self.pyramid_pooling(E1)
        S4 = self.sfa1(E1, S5, img)
        S3 = self.sfa2(E2, S4, img)
        S2 = self.sfa3(E3, S3, img)
        S1 = self.sfa4(E4, S2, img)
        S4 = self.up1(S4)
        S3 = self.up2(S3)
        S2 = self.up3(S2)
        S1 = self.up4(S1)
        output = torch.cat([S4, S3, S2, S1], dim=1)
        output = self.CBM(output, img)
