...
class SFTA(nn.Module):
    ...
    def forward(self, x, y, cls_token=None):
        ...
        # if ref != None:
        #     ref = torch.cat((ref[0], ref[1], ref[2], ref[3]), dim=1)
        #     ref_guidence = einops.repeat(ref, "b c -> b c h w", h=H1, w=H1)
        #     first_out = self.ref_fuse(torch.cat((first_out, ref_guidence), dim=1))
        ...
        hidden_states = self.activation(self.pos_linear(hidden_states))
