...
def weight_init(module):
    for n, m in module.named_children():
        if isinstance(m, nn.Conv2d):
            ...
            weight_init(m)
        elif isinstance(m, (nn.ReLU, nn.Sigmoid, nn.Softmax, nn.PReLU, nn.AdaptiveAvgPool2d, nn.AdaptiveMaxPool2d, nn.AdaptiveAvgPool1d, nn.Sigmoid, nn.Identity, nn.UpsamplingBilinear2d)):
            pass
        else:
            m.initialize()
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.predtrans1  = nn.Sequential(nn.Conv2d(channels, 1, kernel_size=3, padding=1), nn.Sigmoid())
        self.predtrans2  = nn.Sequential(nn.Conv2d(channels, 1, kernel_size=3, padding=1), nn.Sigmoid())
        self.predtrans3  = nn.Sequential(nn.Conv2d(channels, 1, kernel_size=3, padding=1), nn.Sigmoid())
        self.predtrans4  = nn.Sequential(nn.Conv2d(channels, 1, kernel_size=3, padding=1), nn.Sigmoid())
        self.predtrans5  = nn.Sequential(nn.Conv2d(channels, 1, kernel_size=3, padding=1), nn.Sigmoid())
