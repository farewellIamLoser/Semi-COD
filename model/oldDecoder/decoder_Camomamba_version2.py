...
class SFTA(nn.Module):
    def __init__(self, x_channels, y_channels, channels, img_size, output_img_size, stride, in_pos=None, out_pos=None, is_fisrt_cls=False):
        ...
        self.drop_path_x = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()
        self.drop_path_y = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()
        self.drop_path_xy = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()
        self.drop_path_yx = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()
        self.drop_path_final = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()
        self.norm_f_x = (nn.LayerNorm if not rms_norm else RMSNorm)(
            embed_dim, eps=norm_epsilon
        )
        self.norm_f_y = (nn.LayerNorm if not rms_norm else RMSNorm)(
            embed_dim, eps=norm_epsilon
        )
        self.norm_f_xy = (nn.LayerNorm if not rms_norm else RMSNorm)(
            embed_dim, eps=norm_epsilon
        )
        self.norm_f_yx = (nn.LayerNorm if not rms_norm else RMSNorm)(
            embed_dim, eps=norm_epsilon
        )
        ...
        if in_pos is not None:
            self.pos_linear = nn.Linear(in_pos, out_pos)
        self.pos_linear_xy = nn.Linear(embed_dim * 3, embed_dim)
        self.pos_linear_yx = nn.Linear(embed_dim * 3, embed_dim)
        self.activate_xy = nn.ReLU()
        self.activate_yx = nn.ReLU()
        if self.is_cls:
            self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.patch_embed_x = PatchEmbed(img_size=img_size, patch_size=patch_size, stride=stride * 2, in_chans=x_channels, embed_dim=embed_dim)
        self.patch_embed_y = PatchEmbed(img_size=img_size, patch_size=patch_size, stride=stride * 2, in_chans=y_channels, embed_dim=embed_dim)
        self.unpatch_embed = UnpatchEmbed(
            img_size=output_img_size, patch_size=patch_size, stride=stride, embed_dim=embed_dim, in_chans=channels)

    def forward(self, x, y):

        if self.is_cls == True:
            hidden_states_x = self.patch_embed_x(x)
            hidden_states_y = self.patch_embed_y(y)
            cls_token = self.cls_token.expand(hidden_states_x.shape[0], -1, -1)
            hidden_states_x = torch.cat((cls_token, hidden_states_x), dim=1)
            hidden_states_y = torch.cat((cls_token, hidden_states_y), dim=1)
        else:
            hidden_states_x = self.patch_embed_x(x)
            hidden_states_y = y
            cls_token = hidden_states_y[:, 0, :].unsqueeze(1)
            hidden_states_x = torch.cat((cls_token, hidden_states_x), dim=1)
        ...
        for layer in self.x_layers:
            hidden_states_x, residual_x = layer(hidden_states_x, residual_x)
        hidden_states_x = rms_norm_fn(
            self.drop_path_x(hidden_states_x),
            self.norm_f_x.weight,
            self.norm_f_x.bias,
            eps=self.norm_f_x.eps,
            residual=residual_x,
            prenorm=False,
            residual_in_fp32=self.residual_in_fp32,
        )
        ...
        for layer in self.y_layers:
            hidden_states_y, residual_y = layer(hidden_states_y, residual_y)
        hidden_states_y = rms_norm_fn(
            self.drop_path_y(hidden_states_y),
            self.norm_f_y.weight,
            self.norm_f_y.bias,
            eps=self.norm_f_y.eps,
            residual=residual_y,
            prenorm=False,
            residual_in_fp32=self.residual_in_fp32,
        )
        if self.is_cls is not True:
            hidden_states_y = hidden_states_y.permute(0, 2, 1)
            hidden_states_y = self.pos_linear(hidden_states_y)
            hidden_states_y = hidden_states_y.permute(0, 2, 1)

        states_x = torch.sigmoid(hidden_states_x)
        states_y = torch.sigmoid(hidden_states_y)
        hidden_states_xy_bg = hidden_states_x * (1 - states_y)
        hidden_states_xy_fg = hidden_states_x * states_y
        hidden_states_xy_n = hidden_states_x
        hidden_states_xy = torch.cat((hidden_states_xy_fg, hidden_states_xy_bg, hidden_states_xy_n), dim=2)
        hidden_states_yx_bg = hidden_states_y * (1 - states_x)
        hidden_states_yx_fg = hidden_states_y * states_x
        hidden_states_yx_n = hidden_states_y
        hidden_states_yx = torch.cat((hidden_states_yx_fg, hidden_states_yx_bg, hidden_states_yx_n), dim=2)
        hidden_states_yx = self.activate_yx(self.pos_linear_yx(hidden_states_yx))
        hidden_states_xy = self.activate_xy(self.pos_linear_xy(hidden_states_xy))
        # xy mamba strengthen
        residual_xy = None
        for layer in self.xy_layers:
            hidden_states_xy, residual_xy = layer(hidden_states_xy, residual_xy)
        hidden_states_xy = rms_norm_fn(
            self.drop_path_xy(hidden_states_xy),
            self.norm_f_xy.weight,
            self.norm_f_xy.bias,
            eps=self.norm_f_xy.eps,
            residual=residual_xy,
            prenorm=False,
            residual_in_fp32=self.residual_in_fp32,
        )
        ...
        # yx mamba strengthen
        residual_yx = None
        for layer in self.yx_layers:
            hidden_states_yx, residual_yx = layer(hidden_states_yx, residual_yx)
        hidden_states_yx = rms_norm_fn(
            self.drop_path_yx(hidden_states_yx),
            self.norm_f_yx.weight,
            self.norm_f_yx.bias,
            eps=self.norm_f_yx.eps,
            residual=residual_yx,
            prenorm=False,
            residual_in_fp32=self.residual_in_fp32,
        )
        hidden_states_final = hidden_states_xy + hidden_states_yx

        residual_final = None
        for layer in self.final_layers:
            hidden_states_final, residual_final = layer(hidden_states_final, residual_final)
        hidden_states_final = rms_norm_fn(
            self.drop_path_final(hidden_states_final),
            self.norm_f_final.weight,
            self.norm_f_final.bias,
            eps=self.norm_f_final.eps,
...
class FinalOutput(nn.Module):
    def __init__(self, img_size):
        ...
        in_channel = 64
        self.unpatch_embed = UnpatchEmbed(
            img_size=img_size, patch_size=patch_size, stride=1, embed_dim=embed_dim, in_chans=in_channel)
        self.final_out = nn.Sequential(nn.Conv2d(in_channel, in_channel, 1, bias=False),
                                 nn.BatchNorm2d(in_channel),
                                 nn.ReLU(inplace=True),
                                 nn.Conv2d(in_channel, 1, 1),
    ...
    def forward(self, x):
        x = x[:, 1:, :]
        out = self.unpatch_embed(x)
        out = self.upsample(out)
        out = F.interpolate(out, size=320)
        out = self.final_out(out)
        return out
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        img_sizes = [10, 20, 40, 80, 160, 320]
        output_img_sizes = [7, 12, 22, 42, 320]

        self.pyramid_pooling = PyramidPooling(512, channels)
        self.sfa1 = SFTA(512, channels, channels, img_size=img_sizes[0], output_img_size=output_img_sizes[0], stride=1,
                         is_fisrt_cls=True)
        self.sfa2 = SFTA(320, channels, channels, img_size=img_sizes[1], output_img_size=output_img_sizes[1], stride=1,
                         in_pos=17, out_pos=82, is_fisrt_cls=False)
        self.sfa3 = SFTA(128, channels, channels, img_size=img_sizes[2], output_img_size=output_img_sizes[2], stride=1,
                         in_pos=82, out_pos=362, is_fisrt_cls=False)
        self.sfa4 = SFTA(64, channels, channels, img_size=img_sizes[3], output_img_size=output_img_sizes[3], stride=1,
                         in_pos=362, out_pos=1522, is_fisrt_cls=False)
        ...
        self.class_cate = nn.Sequential(
            ...
            nn.Conv2d(channels * 4, channels * 32, 1),
            nn.ReLU(True),
            nn.Conv2d(channels * 32, 69 * 4, 1)
        )
        ...
        self.final_output = FinalOutput(42)
        self.CBM = CombineBlockMamba(channels * 4, 1, img_sizes[5], stride=4)
    ...
    def forward(self, E1, E2, E3, E4):
        ...
        output = self.final_output(S1)
        return output, cate_pred