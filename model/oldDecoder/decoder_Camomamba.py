...
class SFTA(nn.Module):
    def __init__(self, x_channels, y_channels, channels, img_size, output_img_size, next_size, stride, is_fisrt_cls=False):
    ...
    def forward(self, x, y, cls_token=None):
        ...
        hidden_states_xy = torch.cat((hidden_states_x, (1 - hidden_states_y)), dim=1)
        hidden_states_yx = torch.cat((hidden_states_y, (1 - hidden_states_x)), dim=1)
        ...
        hidden_states_final = torch.cat((hidden_states_xy, hidden_states_yx), dim=1)
        if self.is_cls:
            cls_token = self.cls_token.expand(hidden_states_x.shape[0], -1, -1)
            hidden_states_final = torch.cat((cls_token, hidden_states_final), dim=1)
        else:
            hidden_states_final = torch.cat((cls_token, hidden_states_final), dim=1)
        residual_final = None
        ...
        cls_token = hidden_states_final[:, 0, :].unsqueeze(1)
        next_out = hidden_states_final[:, 1:, :]
        out = self.unpatch_embed(next_out)
        out = F.interpolate(out, size=self.next_size, mode='bilinear', align_corners=True)
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        output_img_sizes = [11, 21, 41, 81, 320]

        self.pyramid_pooling = PyramidPooling(512, channels)
        self.sfa1 = SFTA(512, channels, channels, img_size=img_sizes[0], output_img_size=output_img_sizes[0], next_size=img_sizes[1], stride=1, is_fisrt_cls=True)
        self.sfa2 = SFTA(320, channels, channels, img_size=img_sizes[1], output_img_size=output_img_sizes[1], next_size=img_sizes[2], stride=1, is_fisrt_cls=False)
        self.sfa3 = SFTA(128, channels, channels, img_size=img_sizes[2], output_img_size=output_img_sizes[2], next_size=img_sizes[3], stride=1, is_fisrt_cls=False)
        self.sfa4 = SFTA(64, channels, channels, img_size=img_sizes[3], output_img_size=output_img_sizes[3], next_size=img_sizes[4], stride=1, is_fisrt_cls=False)
