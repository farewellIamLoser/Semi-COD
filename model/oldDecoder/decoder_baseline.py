...
class SFTA(nn.Module):
    def __init__(self, in_channel1, in_channel2, out_channel):
        ...
        self.down_channel1 = nn.Sequential(
            conv3x3(in_channel1, out_channel, stride=1),
            nn.BatchNorm2d(out_channel),
            nn.ReLU()
        )
        self.down_channel2 = nn.Sequential(
            conv3x3(in_channel2, out_channel, stride=1),
            nn.BatchNorm2d(out_channel),
            nn.ReLU()
        )
    ...
    def forward(self, x, y):
        ...
        y = self.down_channel2(y)
        y = F.interpolate(y, size=(H1, H1), mode='bilinear', align_corners=True)
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.sfa1 = SFTA(320, 512, channels)
        self.sfa2 = SFTA(128, 128, channels)
        self.sfa3 = SFTA(64, 128, channels)
        self.class_cate = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Conv2d(channels * 6, channels * 64, 1),
            nn.ReLU(True),
            nn.Conv2d(channels * 64, 69 * 4, 1)
        )
        self.final_out = nn.Sequential(nn.Conv2d(channels * 6, channels, 1, bias=False),
        ...
        self.up3 = nn.UpsamplingBilinear2d(scale_factor=16)
        # self.initialize()
    def forward(self, E1, E2, E3, E4):
        ...
        S2 = self.up2(S2)
        S1 = self.up1(S1)
        output = torch.cat((S1, S2, S3), 1)
        final_output = self.final_out(output)
        cate_pred = self.class_cate(output)
        cate_preds = cate_pred.chunk(4, dim=1)
