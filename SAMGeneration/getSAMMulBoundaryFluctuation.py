...
def get_mask(image, fig_mask, flip=None, angle=None, scale=None):
    ...
    unique_colors = count_instance(fig_mask)
    all_cooridinates = []
    all_cooridinate = []
    for color in unique_colors:
        if np.array_equal(color, np.array([0., 0., 0.])):
            continue
        all_row_indices, all_col_indices = np.where((fig_mask[:, :, 0] == color[0])&(fig_mask[:, :, 1] == color[1])&(fig_mask[:, :, 2] == color[2]))
        all_cooridinate.append(list(zip(all_col_indices, all_row_indices)))
    for lis in all_cooridinate:
        all_cooridinates.extend(lis)
    ins_result = {}
    for color in unique_colors:
        if np.array_equal(color, np.array([0., 0., 0.])):
            continue
        if np.array_equal(color, np.array([1., 1., 1.])):
            continue
        fg_row_indices, fg_col_indices = np.where((fig_mask[:, :, 0] == color[0])&(fig_mask[:, :, 1] == color[1])&(fig_mask[:, :, 2] == color[2]))
        ...
        # 获取背景点坐标
        bg_coordinates = [x for x in all_cooridinates if x not in fg_coordinates]
        ...
        ins_result[tuple(color)] = result
    return ins_result, unique_colors

def mask_select(return_masks, unique_colors, H, W):
    color_ins = {}
    multi_mask = np.zeros((H, W, 3))
    for i, return_mask in return_masks.items():
        for color, mask in return_mask.items():
            if color in color_ins:
                color_ins[tuple(color)].append(mask)
            else:
                color_ins[tuple(color)] = [mask]

    for color, color_masks in color_ins.items():
        ssim_color_mask = np.zeros((H, W))
        for i, (color_mask_i) in enumerate(color_masks):
            a = 0
            for j, (color_mask_j) in enumerate(color_masks):
                if i != j:
                    a += ssim(color_mask_i, color_mask_j)
            ssim_color_mask += np.where(color_mask_i > 0, 1, color_mask_i) * a

        nonzero_values = ssim_color_mask[ssim_color_mask != 0]
        average_nonzero = np.mean(nonzero_values)
        position = ssim_color_mask > average_nonzero
        multi_mask[position] = color
...
def get_random_mask(image_files):
    ...
    for i in range(epoch):
        if i == 0:
            ...
            return_mask, unique_colors = get_mask(flipped_image, flipped_mask, flip=True)
        ...
        if i >= 1 and i <= 3:
            ...
            return_mask, unique_colors = get_mask(rotated_image, rotated_mask, angle=return_angle)

        if i == 4:
            return_mask, unique_colors = get_mask(sam_image, mask)
        ...
        if i > 4:
            ...
            return_mask, unique_colors = get_mask(normal_sam_image, normal_mask, scale=(W, H))
        return_masks[i] = return_mask
    multi_mask = mask_select(return_masks, unique_colors, H, W)
...
if __name__ == '__main__':
    ...
    fold = r'SAMOutput\epoch7+pick20'
