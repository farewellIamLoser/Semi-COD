...
sam_checkpoint = "/mnt/e/Mr.Wu/codes/Weakly-Supervised-Camouflaged-Transformer/pretrained/sam_vit_h_4b8939.pth"
model_type = "vit_h"
device = 'cuda'
...
def get_mask(image, fig_mask, flip=None, angle=None, scale=None):
    ...
    fg_row_indices, fg_col_indices = np.where((fig_mask[:, :, 0] == 254)&(fig_mask[:, :, 1] == 254)&(fig_mask[:, :, 2] == 254))
    if len(fg_col_indices) <= 9:
        fg_row_indices, fg_col_indices = np.where(
            (fig_mask[:, :, 0] > 0) & (fig_mask[:, :, 1] > 0) & (fig_mask[:, :, 2] > 0))
    fg_coordinates = list(zip(fg_col_indices, fg_row_indices))
    coordinates = nine_grid_sampling(fg_coordinates)
    num_fg_points = len(coordinates)
    coordinates_labels = np.ones(num_fg_points)
    ...
    for i, (return_mask_i) in enumerate(return_masks):
        ...
        for j, (return_mask_j) in enumerate(return_masks):
            if i != j:
                ...
                grad_j = MultiDirGrad(return_mask_i, return_mask_j) * (1 - a)
                grad += 1 - grad_j / (grad_j.max() + 1e-10)
...
def get_random_mask(image_files):
    ...
    mask_path = image_files.replace('Image', 'Prompt').replace('jpg', 'png')
...
def process_image(path):
    ...
    for image_files in image_files:
        try:
            ...
            maskpath_save = mask_path.replace('Prompt', fold)
