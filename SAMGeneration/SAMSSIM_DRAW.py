...
def mask_select(return_masks, H, W):
    ...
    for i, (return_mask_i) in enumerate(return_masks):
        ...
        ssim_masks.append(np.where(return_mask_i > 0, 1, return_mask_i) * a)
    return ssim_color_mask, ssim_masks

def get_random_mask(image_files, fold):
    epoch = 7
    shape = 768
    ...
    return_masks = []
    mask_path = mask_path.replace('oldScribble', fold)
    filename_with_extension = os.path.basename(mask_path)
    file_name, file_extension = os.path.splitext(filename_with_extension)
    new_folder_path = os.path.join(os.path.dirname(mask_path), file_name)
    ...
    for i, matrix in enumerate(return_masks):
        plt.figure(figsize=(8, 8))  # 设置热图尺寸
        plt.imshow(matrix, cmap='gray', interpolation='nearest')  # 使用热图色彩映射  # 添加色彩条 # 图片标题
        plt.axis('off')  # 关闭坐标轴
        plt.tight_layout()
        # 保存热图到文件夹
        filename = os.path.join(new_folder_path, f'heatmap_{i}.png')
        plt.savefig(filename)
        plt.close()  # 关闭当前图形，以便下一次循环

        print(f'Saved {filename}')

    multi_mask, ssim_masks = mask_select(return_masks, H, W)

    for i, matrix in enumerate(ssim_masks):
        ...
        plt.tight_layout()
        # 保存热图到文件夹
        filename = os.path.join(new_folder_path, f'SSIM_MAP{i}.png')
        plt.savefig(filename)
        plt.close()  # 关闭当前图形，以便下一次循环
...
if __name__ == '__main__':
    ...
    fold = r'SAMOutput\MAP_SSIM_HEAP'
    image_files = glob.glob(os.path.join(path, "*.jpg"))
    for image_files in image_files:
        mask, mask_path = get_random_mask(image_files, fold)
