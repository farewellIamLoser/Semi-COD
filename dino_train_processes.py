...
def train_loss(data, mode, net, ctx, ft_dct, w_ft=.1, ft_st = 60, ft_fct=.5, ft_head=True, mtrsf_prob=1, ops=[0,1,2], w_l2g=0, l_me=0.1, me_st=50, me_all=False, multi_sc=0, l=0.3, sl=1):
    ...
    if mode == 'stage1':
        ...
        out1, out2, of1, of2 = net(image, image_scale, mode)
        ...
        stu_loss_ssc = 0
        for stu, tea in zip(of1, of2):
            stu = pre_transform(stu)
            stu_loss_ssc += SaliencyStructureConsistency(stu, tea.detach(), 0.85)
        ...
        all_loss = all_loss2 + all_loss1 + loss_ssc + loss_intro1 + loss_intro2 + stu_loss_ssc
